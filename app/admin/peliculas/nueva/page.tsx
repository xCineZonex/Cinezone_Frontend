...
export default function NuevaPeliculaPage() {
  ...
  const fetchEnums = async () => {
    try {
      const res = await api.get('/admin/catalogo/enums');
      setEnums(res.data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post('/admin/catalogo/peliculas', {
        ...formData,
      ...
      toast.error(error.response?.data?.message || 'Error al crear la película');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-foreground">Nueva Película</h1>
          <p className="text-muted-foreground">Agrega una nueva película al catálogo</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="pt-6 border-t border-border flex justify-end gap-4">
            ...
            >
              {loading ? 'Guardando...' : (
                <>
                  <Save className="w-5 h-5" /> Guardar Película
                </>
              )}
            </button>
          </div>
